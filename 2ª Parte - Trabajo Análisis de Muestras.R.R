...
intervalos.sturges(ing.M)
intervalos.sturges(ing.F)
...
ggplot(data = df.M, aes(x = ingresos, y=..density.., weight = pesos)) +    # Histograma normalizado de los ingresos de empleados masculinos.
  geom_histogram(fill = "#BFEFFF", color = "black",
...
df.F = data.frame(ingresos = ing.F, pesos = w.F)                           # Dataframe con los ingresos y pesos femeninos.

ggplot(data = df.F, aes(x = ingresos, y=..density.., weight = pesos)) +    # Histograma normalizado de los ingresos de empleadas femeninas.
  geom_histogram(fill = "#FFC0CB", color = "black",
  ...
  xlab("Ingresos (euros)") +
  ylab("Densidad") +
  theme(panel.border = element_rect(fill = "transparent", color = "#FFB6C1", size = 2),
        plot.background = element_rect(fill = "#FFFFFF"),
...
ggplot(data = df, aes(group = Sexo, y = ingresos, weight = pesos, fill = Sexo)) +      # Diagramas de caja de los ingresos de ambos sexos.
  ...
  ylab("Ingresos (euros)") +
  theme(panel.border = element_rect(fill = "transparent", color = "#030303", size = 2),
        plot.background = element_rect(fill = "#FFFFFF"),
        panel.background = element_rect(fill = "#FFFFFF"),
        plot.title=element_text(family = "mono", size = 15, hjust = 0.5),
        axis.title.y = element_text(family = "mono", face = "bold"),
